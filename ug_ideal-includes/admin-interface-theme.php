...
<?php $theme_manager = new Theme_manager(); ?>
...
class Theme_manager {
	...
	function __construct() {
		...
		$this->category_id = get_cat_ID('company_contacts');

		$this->getThemePost();

		if (!$this->theme_post) {
			$insert = wp_insert_post([
				'post_name' => 'theme_manager',
				'post_category' => $this->category_id,
				'post_title' => 'Тема интерфейса (цвета)',
				'post_status'=> 'publish'
			]);
			$this->getThemePost();
		}

		if (isset($_POST['theme_manager'])) {
			$update = wp_update_post([
				'ID' => $this->theme_post->ID,
		...
		$this->theme_post_content = json_decode($this->theme_post->post_content);
		
	}

	function getThemePost () {
		$post = get_posts([
			'category_name' => 'company_contacts',
			'name' => 'theme_manager', 
		])[0];
		$this->theme_post = $post;
	}
